...
async function seed() {
    ...
    await User_1.User.deleteMany({ role: { $ne: 'super_admin' } });
    await Subject_1.Subject.deleteMany({});
    await Group_1.Group.deleteMany({});
    await StudentSubject_1.StudentSubject.deleteMany({});
    await ScheduleEvent_1.ScheduleEvent.deleteMany({});
    await Grade_1.Grade.deleteMany({});
    // Groups
    const groups = await Group_1.Group.insertMany([
        { name: 'КН-21' },
        { name: 'КН-22' },
        { name: 'ІПЗ-31' },
    ]);
    ...
    // Teachers
    const teachers = await User_1.User.create([
    ...
    // Students
    const students = await User_1.User.create([
    ...
    // Subjects
    const subjects = await Subject_1.Subject.create([
    ...
    await StudentSubject_1.StudentSubject.insertMany([
    ...
    const day = (offset) => (0, date_fns_1.format)((0, date_fns_1.addDays)(weekStart, offset), 'yyyy-MM-dd');
    // Розклад — кожен викладач в різний час, без перетинів
    await ScheduleEvent_1.ScheduleEvent.insertMany([
    ...
    // Grades
    const gradeDate = (daysAgo) => (0, date_fns_1.format)((0, date_fns_1.addDays)(new Date(), -daysAgo), 'yyyy-MM-dd');
    await Grade_1.Grade.insertMany([
